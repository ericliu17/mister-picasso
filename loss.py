...
def calc_loss_grad(model, combo_img, img_width, img_height):
    ...
    # the loss between the base image and combined image
    base_img_feats = layer_feats[0, :, :, :]
    combo_feats = layer_feats[2, :, :, :]
    cl = content_loss(base_img_feats, combo_feats)
    loss += content_weight * cl

    # calculate style loss in each layer -
    # the loss between the style image and combined image
    for name, output in outputs_dict.iteritems():
